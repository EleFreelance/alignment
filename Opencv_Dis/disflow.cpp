...
#define INF 1E+10F
	...
	DISOpticalFlowImpl::DISOpticalFlowImpl()
	{
		...
		//for (int i = 0; i < max_possible_scales; i++)
			//variational_refinement_processors.push_back(vivoVariationalRefinement::create());
	}
		...
		Mat &dst_I0y, Mat &I0x, Mat &I0y)
	{
		...
		for (int i = 0; i < h; i++)
		{
			...
			for (int j = patch_size; j < w; j++)
			{
				...
				sum_x += (x_row[j] - x_row[j - patch_size]);       //BoxFilter计算块区域求和公式
				sum_y += (y_row[j] - y_row[j - patch_size]);       // sum[i] = sum[i-1] + buff[j+patch_size] - buff[j] 
...
#undef UPDATE_SPARSE_J_COORDINATES
	...
	void DISOpticalFlowImpl::calc(Mat& I0, Mat& I1, Mat& flow)
	{
		if (I0.channels() == 3 && I1.channels() == 3)
		{
			Mat I0_hsv[3],I1_hsv[3];
			split(I0, I0_hsv);
			split(I1, I1_hsv);
			I0 = I0_hsv[0].clone();
			I1 = I1_hsv[0].clone();
		...
		if (flow.depth() == CV_32F && flow.channels() == 2)
			use_input_flow = true;
		else
			flow.create(I1Mat.size(), CV_32FC2);
		Mat flowMat = flow.clone();
